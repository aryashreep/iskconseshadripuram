...
$dbImages = [];
$dbDisappearances = getDonationCauses('disappearance', false);
foreach ($dbDisappearances as $db) {
...
foreach ($disappearances as $d) {
    $item = [
        'title'        => $d['name'],
        'desc'         => $d['desc'],
        'image'        => !empty($d['slug']) ? ($dbImages[$d['slug']] ?? 'assets/images/banners/disappearance/' . $d['slug'] . '.jpg') : 'assets/images/banners/calendar.jpg',
    ];
    if (!empty($d['slug'])) {
        $item['link'] = 'festivals/disappearance/' . $d['slug'] . '/';
...
$listConfig = [
    ...
    'donationSlug'     => 'disappearance-seva-fund',
];
