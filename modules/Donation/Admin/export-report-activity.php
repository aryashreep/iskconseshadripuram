<?php
/**
 * Export Activity-wise Donation Report as CSV
 */
...
$filename = 'isjm_activity_report_' . date('Y-m-d_H-i-s') . '.csv';
...
fputcsv($output, ['Category', 'Activity', 'Number of Donations', 'Total Amount (INR)', '% of Total']);
...
try {
    $sql = "
        ...
        WHERE {$whereClause}
        GROUP BY COALESCE(c.category, 'general'), c.title
